...
Запускает histogram / raw_waveform_snapshots (PNG: 1 с, ½ с, ¼ с, ⅛ с начала)
/ autocorrelation / spectral / entropy и собирает
...
def make_single_report(raw: Path | None, bits: Path | None,
    ...
    if raw and raw.exists():
        ...
        run(
            [
                sys.executable,
                str(THIS_DIR / "raw_waveform_snapshots.py"),
                "--in",
                str(raw),
                "--out-dir",
                str(out_dir),
        ...
        parts.append("### Осциллограмма (начало записи)\n")
        for png, cap in [
            ("scope_time_1s000.png", "Первые 1 с"),
            ("scope_time_500ms.png", "Первые 1/2 с"),
            ("scope_time_250ms.png", "Первые 1/4 с"),
            ("scope_time_125ms.png", "Первые 1/8 с"),
        ]:
            if (out_dir / png).is_file():
                parts.append(f"![{cap}]({png})\n")
